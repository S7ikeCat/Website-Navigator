...
export const ProductyBezSvininy = () => {

    const [storeValues, setStoreValues] = useState([]);
    const db = getFirestore();

    const fetchDataFromFirestore = async () => {
        const querySnapshot = await getDocs(collection(db, "producty_bez_svininy"));
        const temporaryArr = [];
...
<li><Link activeClassName='active-link' to="/sosiski_sardelki" style={{textDecoration: 'none', color: 'black'}}>Сосиски и сардельки</Link></li>
            ...
            <li><Link activeClassName='active-link' to="/producty_bez_svininy" style={{textDecoration: 'none', color: 'black', borderBottom: '2px solid brown'}}>Продукты без свинины</Link></li>
            
        </ul>

            <div className='container-bezSvin'>
                {storeValues.map((item, index) => (
                    <div key={index} className='Goods_category-bezSvin'>
                       <Link to={`/producty_bez_svininy/${item.id}`} target="_blank" rel="noopener noreferrer"><img src={item.image} alt='' /></Link> 
                        <Link style={{textDecoration: 'none',  color: 'black'}}  to={`/producty_bez_svininy/${item.id}`} target="_blank" rel="noopener noreferrer">  <p>{item.name}</p></Link> 
                        {/* <Link to={`/producty_bez_svininy/${item.id}`} target="_blank" rel="noopener noreferrer">{item.partner1}</Link> */}
                    </div>
                ))}
            </div>
            <Routes>
                <Route path="/producty_bez_svininy/:id" element={<SingleGood />} />
            </Routes>
        </div>
    );
};
