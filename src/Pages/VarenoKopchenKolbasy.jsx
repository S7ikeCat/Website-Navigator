...
export const VarenoKopchenKolbasy = () => {

    const [storeValues, setStoreValues] = useState([]);
    const db = getFirestore();

    const fetchDataFromFirestore = async () => {
        const querySnapshot = await getDocs(collection(db, "vareno-kopchen_kolbasy"));
        const temporaryArr = [];
...
<li><Link activeClassName='active-link' to="/sosiski_sardelki" style={{textDecoration: 'none', color: 'black'}}>Сосиски и сардельки</Link></li>
            ...
            <li><Link activeClassName='active-link' to="/vareno-kopchen_kolbasy" style={{textDecoration: 'none', color: 'black', borderBottom: '2px solid brown'}}>Варено-копченые колбасы</Link></li>
            ...
            <div className='container'>
                {storeValues.map((item, index) => (
                    <div key={index} className='Goods_category'>
                       <Link to={`/vareno-kopchen_kolbasy/${item.id}`} target="_blank" rel="noopener noreferrer"> <img src={item.image} alt='' style={{width: '230px'}}/></Link>
                        <Link style={{textDecoration: 'none',  color: 'black'}} to={`/vareno-kopchen_kolbasy/${item.id}`} target="_blank" rel="noopener noreferrer"> <p>{item.name}</p></Link>
                        {/* <Link to={`/vareno-kopchen_kolbasy/${item.id}`} target="_blank" rel="noopener noreferrer">{item.partner1}</Link> */}
                    </div>
                ))}
            </div>
            <Routes>
                <Route path="/vareno-kopchen_kolbasy/:id" element={<SingleGood />} />
            </Routes>
        </div>
    );
};